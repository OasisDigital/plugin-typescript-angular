System.register(["typescript"], function (exports_1, context_1) {
    ...
    function adjustPaths(loader, modu) {
        if (modu.source.indexOf('moduleId') != -1)
            return modu;
        var url = new URL(modu.address);
        var basePathParts = url.pathname.split('/');
        basePathParts.pop();
        var basePath = basePathParts.join('/');
        var baseHref = new URL(loader.baseURL).pathname;
        ...
        modu.source = modu.source
            ...
            .replace(stylesRegex, function (substr, relativeUrls) {
            var urls = [];
            var match;
            while ((match = stringRegex.exec(relativeUrls)) !== null) {
                if (match[2].startsWith('.')) {
                    var adjustedUrl = "'" + basePath + match[2].substr(1) + "'";
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [2, adjustPaths(loader, load).source];
                }
            });
        });
    }
    ...
    var typescript_1, __assign, Logger, logger$1, CompilerHost, logger$2, typescriptRegex, javascriptRegex, jsxRegex, mapRegex, declarationRegex, jsonRegex, logger$3, templateUrlRegex, stylesRegex, stringRegex, logger, host;
    return {
        setters: [
